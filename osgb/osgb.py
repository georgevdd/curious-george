...
GridSquare = dict((code, (E, N))
    for (N, codes) in enumerate(prefixes)
    for (E, code) in enumerate(codes))


def ParseGridRef(ref):
    '''Turn a ref of the form XXYYYYYYYY into E, N form.
    YYYYYYYY may have 6, 8 or 10 digits.
    '''
    NE = ref[2:]
    n, bad = divmod(len(NE), 2)
    if bad:
        raise ValueError('Grid ref "%s" should have an even number of digits.')
    scale = 10**(5-n)
    (N, E) = int(NE[:n], 10) * scale, int(NE[n:], 10) * scale

    square_code = ref[:2]
    if not square_code.isalpha():
        raise ValueError('Grid ref "%s" should start with two letters.')
    (NN, EE) = GridSquare[square_code]

    return (NN*100000 + N, EE*100000 + E)
